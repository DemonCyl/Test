...
/**
 * Created by mark_yang on 2018/7/11.
 */
public class NutsDataGrabService extends BaseService  {
    private static final Logger logger = LogManager.getLogger(PassDeviceTask.class);
    UI_InitVO uiVO;
    DataHandler dh;
    Set<String> set=null;

    public NutsDataGrabService(UI_InitVO uiVO) {
        this.uiVO = uiVO;
        dh = new DataHandler(uiVO);
    }

    public String insertNutsData(List<NutsDataModel> nutsDataModels) throws Exception {
        String result;
        //System.out.println(LoadBean.toString());
        if(nutsDataModels==null){
        ...
        for (int i=0;i<nutsDataModels.size();i++) {
         String sql="insert into MES_THIRTEENNUTS_LINK VALUES (sys_guid(),'" + nutsDataModels.get(i).getMCE() + "','" + nutsDataModels.get(i).getSN1() + "','" + nutsDataModels.get(i).getMACHINE_ID() +  "',sysdate,to_date('"  + nutsDataModels.get(i).getINTIME()+"','yyyy-MM-dd hh24:mi:ss'))";
    ...
    public String findNewTime(String meachine){
        String time=null;
            String sql = "select * from(select TESTDATE FROM MES_THIRTEENNUTS_LINK where  MACHINE_ID= '"+meachine+"' order by TESTDATE DESC) where rownum=1 ";
        ...
        try {
            ...
            if (ht!=null) {
               ...
               // time =ht.get("TESTDATE").toString();
                SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                time = formatter.format(ht.get("TESTDATE"));
                return time;
            }
