 ...
 * $Id: Barcode_1709_AllPrint.java,v 1.4 2017/09/25 03:50:51 Lucy6_Lu Exp $
 */
...
/**
 * The <code>Barcode_1709_AllPrint</code> class.
 *
 * @version     $Name:  $, $Id: Barcode_1709_AllPrint.java,v 1.4 2017/09/25 03:50:51 Lucy6_Lu Exp $
 * @author      <a href="mailto:william_tsai@Pegatroncorp.com">William Tsai</a>
 * @history      [>: General, +: New, #: Modification, -: Remove, ~: Bug]
 *
 */
public class Barcode_1709_AllPrint extends BasePanel{
	
	
    public static final String VERSION    = "$Name:  $, $Id: Barcode_1709_AllPrint.java,v 1.4 2017/09/25 03:50:51 Lucy6_Lu Exp $";
    public static final String COPYRIGHT  = "Copyright (c) 2008 PEGATRON Inc. All Rights Reserved.";
	Barcode_1709_AllPrint self;
	Font						f14;
	Font						f13;
	Font						f12;
	...
	JTextArea jta;
	String a3;
	public Barcode_1709_AllPrint(UI_InitVO uiVO) {
		super(uiVO);
		init(); 
	}
	void init(){
	...
	public void print() {
		...
		if(!isMatcherPatternStr(JTQty.getText().trim(),"[0-9]")){
			...
			Printdo(jcboxPrint.isSelected(),printNum);
	...
	public PdfPCell createBarCode(PdfWriter writer, String codeStr, int maxWidth) {
		...
		barcodeCell.setBorder(2);
		return barcodeCell;
	}
	private void Printdo(boolean isPrint,String printNum){
		...
		try{
			...
			for(int i=1;i<=num;i++){
				...
				ZXingCode.getLogoQRCode("P" + JTVenC.getText().trim() + " 1P" + JTStage.getText().trim() + " Q" + JTQty.getText().trim() + " V" + JTlc.getText().trim()
				 + " 1T" + JTBt.getText().trim() + " 9D" + a3 + " M" + JTSite.getText().trim() + " 4L" + JTBtend.getText().trim(), null, null);
				Image img = Image.getInstance("D:/test.png");
				img.scalePercent(90f);
				img.setAbsolutePosition(115,155);
				document.add(img);
				ZXingCode.getLogoQRCode("1D" + a3 + " 1Q" + JTQty.getText().trim(), null, null);
				Image img2 = Image.getInstance("D:/test.png");
				img2.scalePercent(90f);
				img2.setAbsolutePosition(135f,41f);
				document.add(img2);
	...
	public void PdfTablecreate(PdfWriter writer,String mmo) {
		...
		try{
			...
			PdfPCell nkkk = new PdfPCell(createBarCode(writer, JTVenC.getText().trim(), a[1]));
			nkkk.setBorder(0);
			nkkk.setColspan(2);
			pdftable.addCell(nkkk);
	...
	public void createPdfTable(PdfWriter writer,String mmo) {
		...
		try{
			...
			PdfPCell Manu = new PdfPCell(createBarCode(writer, JTVenC.getText().trim(), a[1]));
			Manu.setBorder(0);
			Manu.setColspan(3);
			pdftable.addCell(Manu);
			...
			PdfPCell edsf4 = new PdfPCell(createBarCode(writer,a3, a[1]));
			edsf4.setBorder(0);
			edsf4.setColspan(1);
			...
			PdfPCell cc = new PdfPCell(createCell("Date Code1:" + a3,f14,0,0));
			cc.setBorder(0);
			cc.setColspan(1);
			pdftable.addCell(cc);
			
			PdfPCell xx = new PdfPCell(createCell("QTY:" + JTQty.getText().trim(),f14,0,0));
			xx.setBorder(0);
			xx.setColspan(2);
			pdftable.addCell(xx);
