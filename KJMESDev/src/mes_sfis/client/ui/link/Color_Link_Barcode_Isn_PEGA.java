		...
		if(Query_Type == _BarCode_Query){
			try{
				...
				if(StrBarcodeResult.equals("LK")){
					SoundUtil.PlayNo();
					setMessage("錯誤！！！該出貨條碼已LINK");
					JTAMessage.setText("錯誤！！！該出貨條碼已LINK");
					JOptionPane.showMessageDialog(null,"錯誤！！！該出貨條碼已LINK");
					alertmess(Query_Type);
		...
		if(Query_Type == _ISN_Query){
			try{
				...
				if(!isQM7.equals("QMB")){
					SoundUtil.PlayNo();
					setMessage("非QMB站點");
					JTAMessage.setText("非QMB站點");
					JOptionPane.showMessageDialog(null,"S非QMB站點");
					alertmess(Query_Type);
					return;
				}
				...
				/*if(StrOutResult.equals("NON")){
					setMessage("該ISN尚未定義 Link 站點 ");
					JTAMessage.setText("該ISN尚未定義 Link 站點 ");
					alertmess(Query_Type);
					return;
				}
				String part_no = (String)ht.get("PART_NO");
			  	if(part_no != null){
					...
					JTAMessage.setText("出貨條碼料號("+part_no+")與ISN料號不一致("+vITEM+") ");
					alertmess(Query_Type);
					return;
			  	}*/
				toUpdateStep();
